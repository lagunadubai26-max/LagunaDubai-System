...
(function () {
  ...
  if (role === 'Employee' && employeeRestricted.includes(page)) {
  ...
  if (roleEl) {
    if (role === 'Admin') roleEl.textContent = 'كاشير';else roleEl.textContent = 'موظف';
  }
  ...
  if (role === 'Employee') {
    document.querySelectorAll('.sidebar nav a.no-employee').forEach(function (el) {
  ...
  if (span) {
    if (role === 'Admin') span.textContent = 'كاشير';else span.textContent = 'موظف';
  }

  // Verify user has a mapping in user_mappings collection
  _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee() {
    var firebaseUid, snap, allUsers, _t;
    return _regenerator().w(function (_context) {
      while (1) switch (_context.p = _context.n) {
        case 0:
          _context.p = 0;
          firebaseUid = FB && FB.getUid ? FB.getUid() : null;
          if (firebaseUid) {
            _context.n = 1;
            break;
          }
        ...
        case 1:
          _context.n = 2;
          return FB.getDb().collection('user_mappings').doc(firebaseUid).get();
        case 2:
          snap = _context.v;
          if (snap.exists) {
            _context.n = 4;
